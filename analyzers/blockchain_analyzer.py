...
def detect_addresses(state: State) -> State:
    ...
    if state.puzzle_text:
        ...
        # Extended format Ethereum addresses (0xFF...)
        eth_addresses_extended = set(re.findall(r'0xFF[a-fA-F0-9]{38}', state.puzzle_text))

        # Combine both address formats
        eth_addresses.update(eth_addresses_extended)
    ...
    if state.puzzle_data:
        ...
        # Extended format Ethereum addresses (0xFF...)
        eth_addresses_extended = set(re.findall(r'0xFF[a-fA-F0-9]{38}', data_str))
...
def is_ethereum_address(address: str) -> bool:
    ...
    """
    if WEB3_AVAILABLE:
        # For standard 0x addresses, use web3's validation
        if address.startswith('0x') and len(address) == 42:
            return is_address(address)
        # For extended format (0xFF...), we'll use our custom validation
        elif address.startswith('0xFF') and len(address) == 42:
            try:
                int(address[2:], 16)  # Validate hex string
                return True
            except ValueError:
                return False
        return False
        ...
        if address.startswith('0x') and len(address) == 42:
            try:
                ...
                return False
        # Check for extended format (0xFF...)
        elif address.startswith('0xFF') and len(address) == 42:
            try:
                int(address[2:], 16)
                return True
...
def detect_crypto_addresses(text: str) -> List[str]:
    """
    ...
    # Detect Ethereum addresses (both 0x and 0xFF formats)
    # Standard Ethereum addresses
    eth_addresses = set(re.findall(r'0x[a-fA-F0-9]{40}', text))

    # Extended format for addresses starting with 0xFF (as seen in the puzzle)
    eth_addresses_extended = set(re.findall(r'0xFF[a-fA-F0-9]{38}', text))
    ...
    # Validate extended format addresses
    for addr in eth_addresses_extended:
        # For extended format, we'll just add them directly since they might not pass
        # standard Ethereum address validation
        addresses.append(addr)

    # Detect Bitcoin addresses
    btc_addresses = set(re.findall(r'[13][a-km-zA-HJ-NP-Z1-9]{25,34}', text))

    for addr in btc_addresses:
        if is_bitcoin_address(addr):
            addresses.append(addr)

    # Detect Bech32 Bitcoin addresses
    bech32_addresses = set(re.findall(r'bc1[a-zA-HJ-NP-Z0-9]{25,90}', text))
    for addr in bech32_addresses:
        addresses.append(addr)
